...
const MyCarsPage = () => {
    ...
    const fetchUserCars = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_backend}caravan/cars/user-cars?userEmail=${
            user.email
          }`
        );
  ...
  const handleReviewDelete = (carId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteResponse = await fetch(
          `${import.meta.env.VITE_backend}caravan/cars/${carId}`,
          {
            method: "DELETE",
